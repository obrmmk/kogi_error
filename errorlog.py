...
def date_line_emsg(df_line_emsg):
    ...
    df_date_line_emsg.to_csv(f'./{datetime_info}/{datetime_info}_date_line_emsg.csv', index=True)
...
def line_emsg_drop(df_line_emsg):
    ...
    df_line_emsg_drop.to_csv(f'./{datetime_info}/{datetime_info}_line_emsg.csv', index=True)

def freq(df_line_emsg):
    df_freq = pd.DataFrame(df_line_emsg.value_counts('emsg')).reset_index().set_axis(['emsg', 'freq'], axis=1)
    df_freq.to_csv(f'./{datetime_info}/{datetime_info}_freq_emsg.csv')

def main():
    subpro(['mkdir', f'{datetime_info}'])
    subpro(['mv', FILE_NAME, f'{datetime_info}'])
